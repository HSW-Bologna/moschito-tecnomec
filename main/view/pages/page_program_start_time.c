...
static void *create_page(model_t *pmodel, void *extra) {
    ...
    pdata->program    = extra;
    pdata->start_time = model_get_program_start_second(pmodel, pdata->program->erogator, pdata->program->num);
...
static void open_page(model_t *pmodel, void *args) {
    ...
    lv_label_set_text(lbl, view_intl_get_string(pmodel, STRINGS_INIZIO));
            ...
            switch (event.event) {
                case LV_EVENT_CLICKED: {
                    switch (event.data.id) {
                        ...
                        case CONFIRM_BTN_ID:
                            model_set_program_start_second(pmodel, pdata->program->erogator, pdata->program->num,
                                                           pdata->start_time);
                            msg.vmsg.code = VIEW_PAGE_MESSAGE_CODE_BACK;
                            break;

                        case HOUR_MINUS_BTN_ID:
                            if (pdata->start_time > 3600) {
                                pdata->start_time -= 3600;
                            } else {
                                pdata->start_time = 0;
                            }
                            update_page(pmodel, pdata);
                        ...
                        case HOUR_PLUS_BTN_ID:
                            if (pdata->start_time < 3600 * 23) {
                                pdata->start_time += 3600;
                            } else {
                                pdata->start_time = 3600 * 24 - 1;
                            }
                            update_page(pmodel, pdata);
                            break;

                        case MINUTE_MINUS_BTN_ID:
                            if (pdata->start_time > 60) {
                                pdata->start_time -= 60;
                            } else {
                                pdata->start_time = 0;
                            }
                        ...
                        case MINUTE_PLUS_BTN_ID:
                            if (pdata->start_time < 3600 * 24 - 60) {
                                pdata->start_time += 60;
                            } else {
                                pdata->start_time = 3600 * 24 - 1;
                            }
...
static void update_page(model_t *pmodel, struct page_data *pdata) {
    lv_label_set_text_fmt(pdata->lbl_hour, "%02lu", pdata->start_time / 3600);
    lv_label_set_text_fmt(pdata->lbl_minute, "%02lu", (pdata->start_time / 60) % 60);
}
...
const pman_page_t page_program_start_time = {
    .create        = create_page,
